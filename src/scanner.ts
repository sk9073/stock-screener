 ...
 */
export async function findFallingStocks(tickers: string[]): Promise<StockDropResult[]> {
...
if (require.main === module) {
    (async () => {
        try {
            ...
            const fallingStocks = await findFallingStocks(NSE_500_TICKERS);
            
            console.log('\n--- Results ---');
            if (fallingStocks.length === 0) {
                console.log("No stocks found matching the criteria (-6% drop in last 6 days).");
            } else {
                console.table(fallingStocks);
            }
        } catch (error) {
